...
from PySide6.QtGui import QAction, QColor, QKeySequence, QPainter, QPalette, QPen
...
def invert_velocity_profile(
    ...
    def objective(log_vs: np.ndarray) -> float:
        nonlocal eval_count
        eval_count += 1
        vs = np.exp(log_vs)
        try:
            t_calc, _ = forward_model(h, vs, source_offset_m)
        ...
        return data_cost + lam * 100.0 * smooth_cost
...
class WaveformPickerDialog(QDialog):
    ...
    def _on_plot_click(self, event: Any) -> None:
        ...
        if row < 0 or event.inaxes is not self.canvas.axes or event.xdata is None:
            return
...
class RayPathMainWindow(QMainWindow):
    ...
    def _draw_results(self, result: InversionResult) -> None:
        ...
        color_map = self.ray_canvas.figure.get_cmap("viridis")
