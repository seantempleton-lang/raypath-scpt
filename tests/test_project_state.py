...
class ProjectStateTests(unittest.TestCase):
    ...
    def test_manual_project_round_trip_records_versions(self) -> None:
        ...
        try:
            ...
            source._insert_layer_boundary(3.0, "CPT stratigraphic change")
            ...
            with tempfile.TemporaryDirectory() as directory:
                ...
                self.assertEqual(
                    raw_payload["model_parameterization"],
                    "analyst_defined_piecewise_constant_layers",
                )
                self.assertEqual(raw_payload["smoothing_status"], "disabled")
                self.assertNotIn("regularization", raw_payload)

                loaded._load_project(path)
                self.assertAlmostEqual(loaded.offset_spin.value(), 3.25)
                self.assertFalse(hasattr(loaded, "reg_slider"))
                self.assertFalse(hasattr(loaded, "auto_regularization_check"))
                self.assertEqual(loaded.layer_boundary_table.item(0, 0).text(), "3.000")
                self.assertEqual(loaded.layer_boundary_table.item(0, 1).text(), "CPT stratigraphic change")
    ...
    def test_layered_settings_round_trip_and_worker_returns_repeatable_ensemble(self) -> None:
        source = RayPathMainWindow()
        loaded = RayPathMainWindow()
        try:
            source._insert_layer_boundary(7.0, "Upper unit")
            source._insert_layer_boundary(16.0, "Lower unit")
            ...
            self.assertEqual(loaded.layer_boundary_table.rowCount(), 2)
            self.assertEqual(loaded.layer_boundary_table.item(1, 0).text(), "16.000")
            ...
            layer_bottoms = np.asarray([7.0, 16.0, 25.0])
            velocities = np.asarray([180.0, 260.0, 350.0])
            times, _rays, _nodes = forward_layered_model(depths, layer_bottoms, velocities, 2.4)
            ...
            np.testing.assert_allclose(batches[0].layer_bottoms_m, layer_bottoms)
            self.assertEqual(batches[0].results["crossover"].regularization_cost, 0.0)
        finally:
            source._set_dirty(False)
            loaded._set_dirty(False)
            source.close()
    ...
    def test_interval_comparison_tab_draws_both_staggered_phases_and_mean(self) -> None:
        window = RayPathMainWindow()
        try:
            depths = np.arange(0.5, 3.01, 0.5)
            offsets = np.full(depths.size, 2.4)
            arrivals = np.hypot(depths, offsets) / 220.0
            result = InversionResult(
                depths_m=depths,
                thicknesses_m=np.diff(np.r_[0.0, depths]),
                velocities_mps=np.full(depths.size, 220.0),
                observed_times_s=arrivals,
                calculated_times_s=arrivals,
                residuals_s=np.zeros(depths.size),
                ray_parameters=np.zeros(depths.size),
                ray_x_segments=[np.zeros(index + 1) for index in range(depths.size)],
                receiver_offsets_m=offsets,
                rmse_s=0.0,
                success=True,
                message="Synthetic interval comparison",
                iterations=1,
                weighted_rmse_s=0.0,
                data_cost_ms2=0.0,
            ...
            window._draw_interval_comparison(result)

            labels = window.interval_canvas.axes.get_legend_handles_labels()[1]
            self.assertTrue(any("phase A" in label for label in labels))
            self.assertTrue(any("phase B" in label for label in labels))
            self.assertTrue(any("slowness mean" in label for label in labels))
            saved = window._project_payload()["last_result"]["staggered_interval_comparison"]
            json.dumps(window._project_payload(), allow_nan=False)
            self.assertEqual(saved["target_interval_m"], 1.0)
            self.assertEqual(len(saved["phase_a"]["velocities_mps"]), 2)
            self.assertEqual(len(saved["phase_b"]["velocities_mps"]), 2)

            with tempfile.TemporaryDirectory() as directory:
                csv_path = Path(directory) / "interval_comparison.csv"
                window._export_interval_comparison_csv(csv_path)
                with csv_path.open(encoding="utf-8-sig", newline="") as handle:
                    exported = list(csv.DictReader(handle))
            interpretations = {row["Interpretation"] for row in exported}
            self.assertIn("1 m phase A - whole-metre grid", interpretations)
            self.assertIn("1 m phase B - approximately 0.5 m stagger", interpretations)
            self.assertIn("Experimental staggered slowness mean", interpretations)
            self.assertTrue(
                all(row["Arrival Pick Definition"] == window.estimator_combo.currentText() for row in exported)
            )
        finally:
            window._set_dirty(False)
            window.close()
