...
from raypath_scpt import (
    ...
    invert_layered_velocity_profile,
    parse_gru,
    ...
    staggered_interval_velocity_comparison,
    ts1170_5_vs30_band,
...
class InterpretiveLayeredInversionTests(unittest.TestCase):
    def test_exact_two_layer_profile_is_recovered_without_smoothing(self) -> None:
        receivers = np.arange(1.0, 11.0, 1.0)
        bottoms = np.asarray([4.0, 10.0])
        expected = np.asarray([180.0, 320.0])
        offsets = np.full(receivers.size, 2.4)
        arrivals, _rays, _nodes = forward_layered_model(
            receivers, bottoms, expected, 2.4, offsets
        )
        ...
        result = invert_layered_velocity_profile(
            ...
            bottoms,
            receiver_offsets_m=offsets,
            observation_std_s=np.full(receivers.size, 0.0001),
        )

        np.testing.assert_allclose(result.model_layer_velocities_mps, expected, atol=1.0e-6)
        np.testing.assert_allclose(result.model_layer_bottoms_m, bottoms)
        self.assertLess(result.rmse_s * 1000.0, 1.0e-8)
        self.assertEqual(result.regularization_cost, 0.0)
        self.assertEqual(len(result.ray_z_nodes_m), receivers.size)
        np.testing.assert_allclose(result.ray_z_nodes_m[-1], [0.0, 4.0, 10.0])

    def test_model_must_have_fewer_layers_than_observations(self) -> None:
...
class StaggeredIntervalVelocityTests(unittest.TestCase):
    def test_staggered_one_metre_windows_cancel_alternating_pick_error(self) -> None:
        depths = np.arange(0.5, 3.01, 0.5)
        velocity = 200.0
        alternating_error = np.asarray([0.0004, -0.0004, 0.0004, -0.0004, 0.0004, -0.0004])
        arrivals = depths / velocity + alternating_error

        comparison = staggered_interval_velocity_comparison(
            depths,
            depths,
            np.zeros(depths.size),
            arrivals,
            np.full(depths.size, 0.0002),
        )

        self.assertGreater(np.ptp(comparison.adjacent.velocities_mps), 100.0)
        np.testing.assert_allclose(comparison.phase_a.velocities_mps, velocity, atol=1.0e-10)
        np.testing.assert_allclose(comparison.phase_b.velocities_mps, velocity, atol=1.0e-10)
        np.testing.assert_allclose(comparison.slowness_mean_velocities_mps, velocity, atol=1.0e-10)

    def test_interval_velocity_uses_path_length_and_propagates_pick_error(self) -> None:
        depths = np.arange(0.5, 3.01, 0.5)
        offsets = np.full(depths.size, 2.4)
        path_lengths = np.hypot(depths, offsets)
        arrivals = path_lengths / 250.0
        uncertainty = np.full(depths.size, 0.0001)
        ...
        np.testing.assert_allclose(comparison.phase_a.velocities_mps, 250.0, atol=1.0e-10)
        np.testing.assert_allclose(comparison.phase_b.velocities_mps, 250.0, atol=1.0e-10)
        self.assertTrue(np.all(comparison.phase_a.standard_deviations_mps > 0.0))

    def test_non_positive_pick_difference_is_retained_as_an_invalid_audit_window(self) -> None:
        depths = np.asarray([0.5, 1.0, 1.5, 2.0])
        arrivals = np.asarray([0.010, 0.012, 0.011, 0.015])
        ...
        self.assertEqual(comparison.adjacent.velocities_mps.size, 3)
        self.assertFalse(comparison.adjacent.valid_flags[1])
        self.assertTrue(np.isnan(comparison.adjacent.velocities_mps[1]))
        self.assertIn("review picks", comparison.adjacent.status_messages[1])
